...
def analyze_patterns(usage_data, emotion_data):
    ...
    # Filter apps with > 60 mins avg daily
    major_apps = app_global_avg[app_global_avg > 60].index.tolist()
        ...
        for app in pivot.columns:
            ...
            for emo in ['GOOD', 'NORMAL', 'BAD']:
                ...
                if diff_pct > 30: # 30% increase
                    ...
                    emo_kr_map_1 = {"GOOD": "기분이 좋을", "NORMAL": "평범할", "BAD": "기분이 좋지 않을"}
                    emo_kr_1 = emo_kr_map_1.get(emo, emo)

                    title = f"[{emo_kr_1} 때 {cat_name} 사용 ↑]"
                     
                    emo_kr_map_2 = {"GOOD": "기분이 좋은", "NORMAL": "평범한", "BAD": "기분이 좋지 않은"}
                    emo_kr_2 = emo_kr_map_2.get(emo, emo)

                    content = f"{emo_kr_2} 날에는 {app} 사용량이 평소보다 약 {int(diff_pct)}% 증가했습니다."
