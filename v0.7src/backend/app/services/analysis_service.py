...
def get_app_ratios_by_emotion(db: Session, user_id: int):
    ...
    for label, (start_date, end_date) in periods.items():
        ...
        # Main Query: Join AppUsageRaw with daily_emotions
        stats = (
            db.query(
                daily_emotions.c.dominant_emotion,
                AppUsageRaw.package_name,
                func.sum(AppUsageRaw.duration_ms).label("total_ms")
            )
            .join(daily_emotions, AppUsageRaw.usage_date == daily_emotions.c.date)
            .filter(AppUsageRaw.user_id == user_id)
            .group_by(daily_emotions.c.dominant_emotion, AppUsageRaw.package_name)
            .all()
        )
...
def get_usage_by_emotion_status(db: Session, user_id: int):
    ...
    for label, (start_date, end_date) in periods.items():
        ...
        period_result = {
            "GOOD": {"BUSY": 0, "FREE": 0, "NORMAL": 0},
            "NORMAL": {"BUSY": 0, "FREE": 0, "NORMAL": 0},
            "BAD": {"BUSY": 0, "FREE": 0, "NORMAL": 0},
        }
